...
class JPIODFW_Product_List extends WP_List_Table
{
    ...
    function column_name($item)
    {
        ...
        $item_json = esc_attr(wp_json_encode($item));
        $description = esc_attr(isset($item->description) ? $item->description : '');
        $name = esc_attr(isset($item->name) ? $item->name : '');
        $price = esc_attr(isset($item->suggested_price) ? $item->suggested_price : '');
        $store_id = isset($item->store->id) ? absint($item->store->id) : 0;

        $title = '<strong>' . esc_html($item->name) . '</strong>';

        try {
            $actions = [
                'import' => "<a class='btn-dropi-import' data-item='" . $item_json . "' data-description='" . $description . "' data-price='" . $price . "' data-id='" . absint($item->id) . "' data-store='" . $store_id . "' data-name='" . $name . "' href='?page=" . sanitize_text_field($_REQUEST['page']) . "&action=import&product=" . absint($item->id) . "&store=" . $store_id . "&_wpnonce=" . $import_nonce . "'>Importar</a>"
            ];
        } catch (Exception $e) {
    ...
    public function process_bulk_action()
    {
        ...
        if ('import' === $this->current_action()) {
            ...
            if (!wp_verify_nonce($nonce, 'sp_import_product')) {
                ...
                $store = null;
                if (isset($_GET['store'])) {
                    $store = $this->TokenModel->getTokenById(absint(sanitize_text_field($_GET['store'])));
                }

                $result = $this->ProducstInstance->import_product(
                    absint(sanitize_text_field($_GET['product'])),
                    null,
                    null,
                    null,
                    null,
                ...
                if (is_array($result) && !empty($result['success'])) {
                    ...
                    $error = true;
                    $message = is_array($result) && isset($result['message']) ? $result['message'] : __('Error importing product', 'wc-dropi-integration');
                }
    ...
    public function colum_icons($item)
    {
        ...
        $price = esc_attr(isset($item->suggested_price) ? $item->suggested_price : '');
        $store_id = isset($item->store->id) ? absint($item->store->id) : 0;
        $url = "<a class='btn-dropi-import' data-item='" . $item_json . "' data-description='" . $description . "' data-price='" . $price . "' data-id='" . absint($item->id) . "' data-name='" . $name . "' data-store='" . $store_id . "' title='Importar' href='?page=" . sanitize_text_field($_REQUEST['page']) . "&action=import&product=" . absint($item->id) . "&store=" . $store_id . "&_wpnonce=" . $import_nonce . "'><span class='dashicons dashicons-database-import'></span></a>";
